...
def get_neural_network_output(masked_image_white_background):
    ...
    for x,y in locations:
        ...
        windowable_output_image.set_window(x,y,SZ,SZ,Io3Image.from_array(np.array(output_window,np.uint8)))
...
cm = CheckpointManager("pipeline.py",True)
...
def pick_file(db: Database): # Not a checkpoint. Should run once each time the program runs
    ...
    db.engage_subdb(remove_fs_significant_chars(panorama_path).replace(" ","_"))    
